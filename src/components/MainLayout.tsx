...
export function MainLayout({ children }: { children: React.ReactNode }) {
 ...
 if (state.screen === 'CREATION' || state.screen === 'TRIAL_MATCH') {
  return <div className="h-full bg-black text-white flex flex-col font-mono">{children}</div>;
 }

 if (!state.player) {
  return (
   <div className="h-screen w-screen flex bg-black text-white font-mono items-center justify-center p-6 select-none">
    <div className="bg-[#050505] border border-[#222] p-8 max-w-md w-full text-center space-y-5 ">
     ...
     <button
      ...
      className="w-full py-3.5 bg-[#00FF88] hover:bg-white text-black font-black uppercase text-xs tracking-widest transition-all cursor-pointer shadow-[#00FF88]/10 active:scale-98"
     >
      Return to Main Menu
     </button>
    </div>
 ...
 return (
  <div className="h-full w-full flex bg-black text-[#cccccc] font-mono overflow-hidden select-none">
   <Sidebar />
   
   <main className="flex-1 flex flex-col h-full bg-black relative stadium-glow-overlay min-w-0 overflow-hidden">
    
    {/* Landscape Compact Status Bar */}
    <header className="h-12 bg-[#0a0a0a] border-b border-[#222] flex items-center justify-between px-4 sm:px-6 shrink-0 relative z-30">
     ...
     <div className="flex items-center gap-1.5 sm:gap-3">
      ...
      <button 
       ...
       className="h-8 px-2 sm:px-3 border border-[#111] bg-[#050505] hover:border-[#333] active:scale-95 transition-all flex items-center gap-1.5 text-left cursor-pointer"
      >
      ...
      <button 
       onClick={() => setIsDrawerOpen(!isDrawerOpen)}
       className={`w-8 h-8 flex items-center justify-center border border-[#111] bg-[#050505] hover:text-white transition-all cursor-pointer ${
    ...
    {isDrawerOpen && (
     <div className="absolute top-12 left-0 right-0 z-20 bg-[#0a0a0a]/98 border-b border-[#222] p-4 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 animate-in slide-in-from-top duration-200 select-none font-mono">
      
      {/* Calendar Detail Panel */}
      <div className="bg-white/5 border border-[#222] p-3 sm:p-4 flex flex-col justify-between">
       <div>
       ...
       <div className="text-[9px] text-white/40 mt-3 border-t border-[#111] pt-2 flex items-center gap-1.5">
      ...
      <div className="bg-white/5 border border-[#222] p-3 sm:p-4 flex flex-col justify-between">
       ...
       <button
        ...
        className="w-full mt-3 py-1 bg-[#151515] hover:bg-[#202020] border border-[#111] hover:border-[#222] rounded text-[9px] text-white uppercase font-bold tracking-widest transition-all"
       >
        Close Ledger
       </button>
      </div>
