...
export const RealWorldNewsWidget: React.FC<RealWorldNewsWidgetProps> = ({ compact = false }) => {
  ...
  if (compact) {
    return (
      <div className="bg-[#121318] border border-white/10 rounded-xl p-4 shadow-lg font-mono text-xs">
        ...
        {isLoading ? (
          ...
          <div className="space-y-2.5">
            {headlines.slice(0, 3).map((item) => (
              <div key={item.id} className="p-2.5 bg-[#181920] border border-white/5 rounded-lg hover:border-white/20 transition-all">
  ...
  return (
    <div className="bg-[#121318] border border-white/10 rounded-2xl p-5 md:p-6 shadow-2xl font-mono">
      {/* Header with Google Search Grounding Badge */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-b border-white/10 pb-4 mb-4 gap-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border border-teal-500/40 flex items-center justify-center text-teal-400 shrink-0 shadow-lg">
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          ...
          <button
            onClick={loadNews}
            disabled={isLoading}
            className="bg-teal-500/20 border border-teal-500/40 hover:bg-teal-500 hover:text-black text-teal-300 px-3 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 cursor-pointer shadow-md"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredHeadlines.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="bg-[#181920] border border-white/10 hover:border-teal-500/40 rounded-xl p-4 flex flex-col justify-between transition-all group shadow-md"
              >
                <div>
                  ...
                  {item.keyEntities && item.keyEntities.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {item.keyEntities.map((entity, i) => (
                        <span key={i} className="text-[9px] bg-black/50 text-white/50 px-2 py-0.5 rounded border border-white/5">
                          #{entity}
                        </span>
                      ))}
                    </div>
                ...
                <div className="border-t border-white/10 pt-3 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-teal-500/10 hover:bg-teal-500/30 text-teal-300 border border-teal-500/30 text-[10px] font-bold uppercase transition-all flex items-center gap-1 cursor-pointer"
      ...
      {sources && sources.length > 0 && (
        <div className="mt-5 border-t border-white/10 pt-3 flex flex-col md:flex-row items-start md:items-center justify-between gap-2 text-[10px] text-white/40">
