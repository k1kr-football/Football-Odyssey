...
export function OffSeasonScreen() {
  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-6 lg:p-8 custom-scrollbar">
      {/* Header Banner */}
      <div className="bg-[#00FF88]/10 border border-[#00FF88]/30 rounded-xl p-4 sm:p-6 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          return (
            <button
              ...
              className={`p-4 rounded-xl border text-left flex flex-col justify-between transition-all group ${
                canAfford 
                  ? 'bg-[#121212] border-white/10 hover:border-[#00FF88]/50 hover:bg-[#1a1a1a] cursor-pointer' 
                  : 'bg-black/50 border-white/5 opacity-50 cursor-not-allowed'
              }`}
            >
              <div>
                <div className="flex justify-between items-start mb-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${canAfford ? 'bg-[#00FF88]/10 text-[#00FF88]' : 'bg-white/5 text-white/40'}`}>
                    <Icon size={16} />
                  </div>
                  <span className={`text-[10px] font-bold font-mono px-2 py-1 rounded-md ${canAfford ? 'bg-[#00FF88] text-black' : 'bg-white/10 text-white/50'}`}>
      ...
      <div className="flex justify-center mt-auto pb-8">
        <button
          ...
          className="bg-white text-black px-8 py-4 rounded-xl font-black uppercase tracking-widest flex items-center gap-3 hover:bg-[#00FF88] hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:shadow-[0_0_30px_rgba(0,255,136,0.4)]"
        >
      ...
      {showSummary && (
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
          <div className="bg-[#121212] border border-[#00FF88]/30 rounded-2xl p-6 md:p-10 max-w-lg w-full">
            ...
            <button
              ...
              className="w-full bg-[#00FF88] text-black py-4 rounded-xl font-black uppercase tracking-wider hover:bg-white transition-colors"
            >
